...
Systém podporuje dva režimy vytváření instancí: geografický (reálné lokality) a negeografický (syntetické rovinné body).
...
U genetického algoritmu byl výběr rodičů sjednocen na tournament selection. Parametr \texttt{tournament\_k} určuje velikost turnaje, tedy kolik kandidátů vstupuje do lokálního výběru nejlepšího jedince před křížením. Kvalitativní zlepšení výsledků bylo spíše malé, ale tato varianta je lépe řiditelná při ladění parametrů a implementačně čitelnější. Pro křížení je v implementaci použit order-based postup odpovídající OX stylu (kopie souvislého úseku z prvního rodiče a doplnění z druhého rodiče bez duplicit).

U simulovaného žíhání byla v rámci vývoje ověřena i agresivnější strategie práce se stagnací a periodickým návratem teploty inspirovaná externím zdrojem \cite{aaronyw_atsp_py}. V testovaném nastavení této práce však nepřinesla stabilnější ani kvalitnější výsledky než jednodušší varianta s řízeným ochlazováním a fixním výpočetním rozpočtem. Z toho důvodu zůstala v hlavní větvi konzervativnější implementace.

Samostatnou změnou byla formalizace benchmarkovacího workflow. Měření byla sjednocena na master seed, odvozené seedy pro jednotlivé algoritmy a opakované běhy se strukturovaným výstupem. Tím se omezila nahodilost ad hoc testování a bylo možné konzistentně porovnávat kvalitu řešení i rozptyl stochastických metod mezi různými běhy.
...
\begin{itemize}
    ...
    \item u genetického algoritmu je první jedinec populace vždy trasou z greedy heuristiky \emph{Nearest Neighbour} (NN) nad stejnou maticí. Ostatní jedinci vznikají náhodnou permutací vnitřních měst. Po skončení evoluce se na nejlepší trasu aplikuje krátké lokální vylepšení (náhodný budgetovaný 2-opt u STSP, u ATSP operátory \emph{swap}, \emph{relocate} a \emph{or-opt} v modulu \texttt{route\_ops}). Obdobné závěrečné vylepšení se uplatňuje i u ACO, SA a RSO v příslušných souborech v \texttt{tsp\_solver/algorithms/}
