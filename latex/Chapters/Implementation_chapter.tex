...
Jádrem výpočtu je třída \texttt{TSPManager}, která zajišťuje celý výpočetní pipeline: validaci vstupu, volbu efektivní metriky, sestavení matice vzdáleností/časů, spuštění zvoleného solveru, výpočet agregované hodnoty trasy a následné vygenerování geometrie pro mapovou vizualizaci. Detailní výpis je uveden v příloze \ref{sec:appendix_long_listings}, výpis \ref{src:tspmanager_pipeline}.

Jednotné volání konkrétních solverů obstarává \texttt{OptimizationEngine}. Podle zvoleného klíče vybere implementační funkci a předá jí pouze argumenty, které signatura této funkce přijímá. Detailní výpis je uveden v příloze \ref{sec:appendix_long_listings}, výpis \ref{src:optimization_engine_dispatch}.
...
V rámci práce byly ručně připraveny také vlastní geografické instance (krajská města České republiky, případně hlavní města evropských států), které slouží k ověření chování algoritmů na datech blízkých reálné silniční logistice.
...
U GEO TSPLIB importu je implementována kompatibilitní logika: pokud soubor neobsahuje aplikační značku \texttt{COMMENT: MODERN\_GPS\_DIPLOMA}, parser předpokládá historický TSPLIB GEO formát \texttt{DDD.MM} a převádí jej na desetinné stupně. To umožňuje korektní načtení referenčních knihovních instancí bez ručního zásahu.
...
\begin{itemize}
    ...
    \item u genetického algoritmu je první jedinec populace vždy trasou z greedy heuristiky \emph{Nearest Neighbour} (NN) nad stejnou maticí. Ostatní jedinci vznikají náhodnou permutací vnitřních měst. Po skončení evoluce se na nejlepší trasu aplikuje krátké lokální vylepšení (náhodný 2-opt u STSP, u ATSP operátory \emph{swap}, \emph{relocate} a \emph{or-opt} v modulu \texttt{route\_ops}). Obdobné závěrečné vylepšení se uplatňuje i u ACO, SA a RSO v příslušných souborech v \texttt{tsp\_solver/algorithms/}
...
\section{Vizualizace}
Vizualizační vrstva kombinuje \texttt{QWebEngineView} (Leaflet mapa) a PyQt prvky. Piktogramy v uživatelském rozhraní (postranní panel, stavové indikátory, akční tlačítka) vycházejí z kolekce ikon Lucide~\cite{lucide_dev}. Vektorové grafiky ve formátu \textsc{svg} s jednotným stylem.
...
\subsection{Zobrazení nalezeného řešení} Po dokončení výpočtu se do aplikačního stavu ukládá polylinie trasy, pořadí zastávek včetně návratu do výchozího bodu, agregovaná výsledná hodnota odpovídající zvolené metrice (vzdálenost v kilometrech nebo čas v minutách) a klíč použité metriky. Pravý panel nad mapou tuto základní reprezentaci dále rozšiřuje o textové navigační instrukce, výškový profil dostupný v případě, že routovací backend vrátí výšková data, a orientační odhad spotřeby paliva. Samotný výškový profil je vykreslován widgetem postaveným nad \texttt{QPainter}, přičemž vodorovná osa reprezentuje kumulativní vzdálenost trasy a svislá osa odpovídá nadmořské výšce.
...
Tato volba je důležitá z hlediska uživatelského pocitu. GPX je primárně navigační formát, zatímco TSP je optimalizační instance.
