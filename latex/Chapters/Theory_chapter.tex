...
f(n) = g(n) + h(n)
\end{equation}
Tento tvar hodnoticí funkce je přímo uveden v původní práci o A* \cite{hart1968astar}.
Kde:
\begin{itemize}
...
\textit{Složitost a limity:} Standardní DP formulace redukuje naivní $O(n!)$ na $O(n^2 2^n)$ \cite{cmu_15451_dp2_2023}. Počet stavů DP je řádově $n 2^n$, což vede i na vysokou paměťovou náročnost při praktické implementaci.
...
\begin{itemize}
    \item \textbf{Genetické algoritmy (GA):} Tato technika simuluje evoluční proces nad populací kandidátních řešení, kde noví potomci vznikají operacemi křížení a mutace \cite{skanderova2020bia_ex4_ga_tsp}. Pro TSP je zároveň nutné zachovat platnou permutaci měst, tedy bez opakované návštěvy stejného města \cite{skanderova2020bia_ex4_ga_tsp}.
    
    \item \textbf{Mravenčí kolonie (Ant Colony Optimization - ACO):} ACO je inspirováno chováním reálných mravenců a pro TSP používá pravděpodobnostní volbu dalšího města podle feromonu a viditelnosti (inverzní vzdálenosti) \cite{skanderova2020bia_ex8_aco_tsp}. Klíčovým mechanismem je vypařování feromonu a jeho následná aktualizace podle kvality nalezených tras, což umožňuje postupně zvýhodňovat lepší cesty \cite{skanderova2020bia_ex8_aco_tsp}.
    
    \item \textbf{Simulované žíhání (Simulated Annealing - SA):} SA je definováno pomocí řídicích parametrů počáteční teploty, minimální teploty a chladicí konstanty \cite{skanderova2020bia_ex3_sa}. Přijímací pravidlo dovoluje s určitou pravděpodobností přijmout i horší řešení podle členu $e^{-(f(x_1)-f(x))/T}$, takže algoritmus na začátku více prozkoumává prostor řešení a s klesající teplotou se chová více lokálně \cite{skanderova2020bia_ex3_sa}.
...
Knihovna TSPLIB představuje standard pro testování algoritmů řešících TSP \cite{reinelt1994tsplib,tsplib95}. Obsahuje široké spektrum instancí, od vrtání plošných spojů až po simulace měst cesta.
\begin{itemize}
