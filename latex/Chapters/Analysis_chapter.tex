...
\\ \hline
F3 & Systém umožní export vygenerované instance do formátů standardizovaného formátu TSPLIB
\\ \hline
...
\begin{itemize}
    \item Vzhledem k výpočetní náročnosti analýzy větších instancí TSP bude aplikace využívat vícevláknové zpracování v prostředí Qt. Výpočetní a síťově náročné kroky by měly být přesunuty mimo hlavní vlákno uživatelského rozhraní, což udrží aplikaci plynule ovladatelnou
    \item \textbf{Portabilita:} Systém bude implementován v jazyce Python, což zajistí multiplatformní běh (Windows, Linux, macOS)
...
Výpočet matice vzdáleností pro trasové metriky využívá dávkové volání maticových rozhraní služeb ORS a OSRM. Počet vyhodnocovaných dvojic je $n^2-n$, ale v praxi se neřeší samostatným dotazem pro každou dvojici. Matice je sestavována po blocích.
...
\begin{itemize}
    ...
    \item \textbf{Prezentační vrstva:} Kombinuje mapové zobrazení v \texttt{QWebEngineView} s ovládacími panely Flet (později PyQt6)
\end{itemize}
...
V aplikaci je použita kombinace běhu ve worker vlákně a dávkového maticového volání. Síťové requesty jsou v Pythonu synchronní, ale běží mimo hlavní vlákno GUI, takže rozhraní zůstává během výpočtu responzivní.
...
\begin{itemize}
    ...
    \item \textbf{Vlákna v GUI:} Výpočet trasy běží v \texttt{QThread} workeru, takže hlavní vlákno nečeká na síťové operace
