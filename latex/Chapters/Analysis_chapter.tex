...
V této kapitole je popsán proces transformace teoretických poznatků do konkrétního softwarového návrhu. Návrh se zaměřuje na vytvoření modulárního systému s využitím externích geolokačních dat.
...
F2 & Systém provede výpočet matice vzdáleností a časů pomocí mapového API nebo lokálního modelu
\\ \hline
F3 & Systém umožní export vygenerované instance do standardizovaného formátu TSPLIB
\\ \hline
F4 & Systém zajistí vizualizaci vstupních bodů a vypočtené optimální trasy nad mapovým podkladem
\\ \hline
...
\begin{itemize}
    \item Vzhledem k výpočetní náročnosti analýzy větších instancí TSP bude aplikace využívat vícevláknové zpracování. Výpočetní a síťově náročné kroky by měly být přesunuty mimo hlavní vlákno uživatelského rozhraní, což udrží aplikaci plynule ovladatelnou
    \item \textbf{Portabilita:} Systém bude implementován v jazyce, který zajistí multiplatformní běh (Windows, Linux, macOS)
    ...
    \item \textbf{Licenční čistota:} Systém bude prioritně využívat open-source řešení (OSRM, OSM) nevyžadující placené API klíče
...
Diagram případů užití\ref{fig:use_case_diagram} definuje hranice systému a specifikuje interakce mezi uživatelem a jednotlivými funkcionalitami generátoru. Zaměřuje se především na proces konfigurace experimentu, kde uživatel rozhoduje o výběru datového zdroje a optimalizačního kritéria.
...
Diagram komponent \ref{fig:component_diagram} znázorňuje modulární strukturu systému. Centrálním bodem je \textit{TSP Manager}, který koordinuje tok dat mezi uživatelským rozhraním, tvorbou matice a optimalizačním jádrem. Odpovědnosti jsou rozděleny mezi komponenty rozhraní, routovací vrstvu a I/O strategie. Výpočetní jádro \textit{Optimization Engine} je odděleno od síťové vrstvy, takže lze měnit optimalizační metody bez zásahu do routovacích volání.
\begin{figure}[H]
    \centering
...
Aktivitní diagram \ref{fig:activity_diagram} popisuje generování instance od vstupu až po finální export s pomocí externího API. Podobně lze přistupovat i k lokálnímu OSRM modelu.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth,height=0.88\textheight,keepaspectratio]{Figures/TSP_gen_activity_diagram.png}
    \caption{Diagram aktivit v aplikaci}
    \label{fig:activity_diagram}
\end{figure}

Pro aktivitu sestavení matice vzdáleností viz. \ref{fig:activity_diagram} využívá dávkové volání maticových rozhraní služeb ORS a OSRM. Počet vyhodnocovaných dvojic je $n^2-n$, ale v praxi se neřeší samostatným dotazem pro každou dvojici. Matice je sestavována po blocích.
...
Z hlediska softwarového inženýrství je systém navržen s využitím návrhových vzorů, které zajišťují modularitu systému:
...
\begin{itemize}
    ...
    \item \textbf{Prezentační vrstva:} Kombinuje mapové zobrazení s ovládacími panely
\end{itemize}

\section{Návrh datového modelu}
Základním objektem systému je \texttt{TSPInstance}, která vnitřně reprezentuje graf $G=(V, A)$.
...
Klíčovou výzvou při generování reálných TSP instancí je efektivní získávání dat z externích služeb. Úloha je \textit{I/O bound}: čas výpočtu je dán hlavně latencí HTTP komunikace a limity API, ne výkonem CPU.

\subsection{Analýza úzkého hrdla při komunikaci s mapovými API}
Implementace sestavuje matici po blocích přes ORS Matrix API. Počet vyhodnocovaných dvojic je $n(n-1)$, ale neodesílá se samostatný HTTP dotaz pro každou dvojici. Přesto jde o sekvenční síťové volání bloků, kde se na odpověď každého bloku čeká.
Pokud bychom teoreticky uvažovali samostatný request pro každou dvojici a průměrnou latenci jednoho požadavku $t_{lat} = 100$\,ms, pak pro instanci o velikosti $n=100$ (10\,000 tras) vychází:
\begin{equation}
T_{total} = n(n-1) \cdot t_{lat} \approx 10\,000 \cdot 0,1\,\text{s} = 1\,000\,\text{s} \approx 16,7\,\text{min}.
\end{equation}
Tento výpočet je orientační horní odhad. Reálná implementace je rychlejší díky maticovým blokům, ale celková doba je stále ovlivněna latencí, limity služeb a lokálním zpracováním odpovědí.

\subsection{Model neblokujícího získávání matic vzdáleností}
V aplikaci bude použita kombinace běhu ve worker vlákně a dávkového maticového volání. Síťové requesty jsou synchronní, ale běží mimo hlavní vlákno GUI, takže rozhraní zůstává během výpočtu responzivní.

\begin{itemize}
    \item \textbf{Dávkování požadavků:} Kvůli limitu ORS Matrix jsou zdroje a cíle děleny do bloků a výsledná matice je skládána postupně.
    \item \textbf{Vlákna v GUI:} Výpočet trasy běží v workeru, takže hlavní vlákno nečeká na síťové operace.
    \item \textbf{Náhradní režim:} Při chybě ORS volání se systém přepne na lokální OSRM Table API a při selhání obou trasových služeb použije bodovou metriku.
\end{itemize}

\subsection{Sekvenční diagram získávání dat}
Sekvenční diagram komunikace mezi frontendem, backendem (generátorem) a API je z důvodu čitelnosti uveden v příloze na obrázku~\ref{fig:sequence_diagram}. Diagram odpovídá implementaci, kde backend volá bloky matice postupně, čeká na odpověď každého bloku a skládá z nich finální matici.
...
\begin{itemize}
    ...
    \item \textbf{Komunikace po síti:} Knihovna \texttt{requests} a síťové komponenty Qt (\texttt{QNetworkAccessManager}) pro routing i geokódovací volání
    ...
    \begin{itemize}
        ...
        \item \textbf{Open Source Routing Machine:} Lokální náhradní zdroj pro výpočet matic a tras při výpadku primární služby
    \end{itemize}
    \item \textbf{Volba oproti rozhraní Mapy.cz:} Po předchozí domluvě s vedoucím práce bylo pro tuto implementaci zvoleno API ORS. Služba je pro běžné a akademické využití při vlastním API klíči bezplatná, má otevřený ekosystém a veřejnou dokumentaci, klíč lze získat v řádu minut a limity dotazů jsou pro účely práce přijatelné. ORS navíc poskytuje přehledné příklady volání endpointů včetně maticových a directions rozhraní, která odpovídají potřebě generátoru.\cite{ors_matrix_docs,ors_directions_docs}
\end{itemize}
\endinput