...
\\ \hline
F3 & Systém umožní export vygenerované instance do formátů JSON, CSV a standardizovaného formátu TSPLIB %%\cite{26}. 
\\ \hline
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Figures/TSP_gen_activity_diagram.png}
...
Diagram tříd na obrázku \ref{fig:class_diagram} znázorňuje statickou strukturu navrženého systému. Architektura je striktně rozdělena do několika logických celků, které zajišťují oddělení odpovědnosti a vysokou míru rozšiřitelnosti.
\begin{figure}[H]
    \centering
...
\begin{itemize}
    ...
    \item \texttt{distance\_matrix}: Dvourozměrné pole (numpy array) typu $n \times n$.
...
Následující diagram (Obr. \ref{fig:sequence_diagram}) ilustruje interakci mezi frontendem, backendem (generátorem) a API. Je patrné, že generátor nečeká na dokončení prvního požadavku, ale v rychlém sledu odesílá další, přičemž odpovědi zpracovává v pořadí, v jakém přicházejí ze sítě.
